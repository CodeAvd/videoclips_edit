...
def build_outbox_event_query(event_id: UUID) -> Select[tuple[OutboxEvent]]:
    return select(OutboxEvent).where(OutboxEvent.id == event_id).with_for_update()
...
async def renew_outbox_lease(session: AsyncSession, *, event_id: UUID) -> bool:
    result = await session.execute(build_outbox_event_query(event_id))
    event = result.scalars().first()
    if event is None:
        return False
    if event.status != OutboxStatus.claimed:
        return False
    event.lease_expires_at = datetime.now(UTC) + timedelta(seconds=settings.outbox_claim_ttl_seconds)
    await session.flush()
    return True
